...
func newTestbookingsService() (*bookingsService, *MockBookingsRepo, *MockPayment, *MockMovieProvider, *MockRedis) {
	...
	redis := new(MockRedis)
	svc := NewService(repo, redis, pay, nil, movieProv, nil, nil)
...
func Test_deve_listar_filmes_em_cartaz_com_status_especial(t *testing.T) {
	...
	repo.On("GetMoviesPlaying", mock.Anything, "São Paulo", "2026-03-30").Return([]MovieSummary{
		{ID: 1, TMDBID: 550, Title: "Fight Club"},
		{ID: 2, TMDBID: 27205, Title: "Inception"},
	}, nil)
...
func Test_PriceCalculation_Exactness(t *testing.T) {
	...
	repo.On("GetSessionByID", mock.Anything, sessionID).Return(&cinema.Session{
		ID: sessionID, RoomID: roomID, Price: 2999,
		Room: cinema.Room{Type: cinema.RoomTypeVIP},
	}, nil)
