...
func Test_Bookings_Concurrency_SeatSelection_RealRedis(t *testing.T) {
	...
	svc := bookings.NewService(bookingstore.NewStore(db), rdb, mockPayment, mockMailer, mockMovieSvc, nil, nil)
	require.NoError(t, cinemastore.AutoMigrate(db))

	cin := cinemastore.CinemaRecord{Name: "Concurrency Cinema", City: "Test City", Address: "123 Street", Phone: "123", Email: "c@test.com"}
	require.NoError(t, db.Create(&cin).Error)
	require.NotZero(t, cin.ID)

	room := cinemastore.RoomRecord{CinemaID: cin.ID, Name: "Room 1", Capacity: 100, Type: "STANDARD"}
	require.NoError(t, db.Create(&room).Error)
	require.NotZero(t, room.ID)

	movie := moviestore.MovieRecord{TMDBID: 12345, Title: "Concurrency Movie", Runtime: 120, ReleaseDate: time.Now()}
	require.NoError(t, db.Create(&movie).Error)

	session := cinemastore.SessionRecord{MovieID: movie.ID, RoomID: room.ID, StartTime: time.Now().Add(2 * time.Hour), Price: 5000}
	...
	for i := 0; i < numConsumers; i++ {
		uid := uuid.New()
		u := userstore.UserRecord{ID: uid, Username: fmt.Sprintf("user%d", i), Email: fmt.Sprintf("user%d@test.com", i), Password: "hash"}
