...
func (m *MockBookingsRepo) GetCinemaByID(ctx context.Context, id int) (*cinema.Cinema, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*cinema.Cinema), args.Error(1)
}

func (m *MockBookingsRepo) GetSessionsByMovie(ctx context.Context, movieID int, city string, date string) ([]cinema.Session, error) {
	args := m.Called(ctx, movieID, city, date)
	return args.Get(0).([]cinema.Session), args.Error(1)
}

func (m *MockBookingsRepo) GetMoviesPlaying(ctx context.Context, city string, date string) ([]MovieSummary, error) {
	args := m.Called(ctx, city, date)
	return args.Get(0).([]MovieSummary), args.Error(1)
}

func (m *MockBookingsRepo) GetSeatsBySession(ctx context.Context, sessionID int) ([]cinema.Seat, error) {
	args := m.Called(ctx, sessionID)
	return args.Get(0).([]cinema.Seat), args.Error(1)
}
...
func (m *MockBookingsRepo) GetSessionByID(ctx context.Context, sessionID int) (*cinema.Session, error) {
	args := m.Called(ctx, sessionID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockMovieProvider) GetMovieDetails(ctx context.Context, tmdbID int) (*MovieSummary, error) {
