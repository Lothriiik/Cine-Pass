...
func Test_Auth_Login_BruteForce_Protection(t *testing.T) {
	...
	repo := new(auth.MockUserRepo)
	repo.On("GetUserByUsername", mock.Anything, "attacker").Return(nil, auth.ErrInvalidCredentials)

	authSvc := auth.NewAuthService(repo, nil, rdb, nil)
	h := handler.NewHandler(authSvc)

	noopMiddleware := func(next http.Handler) http.Handler { return next }
	r := chi.NewRouter()
	h.RegisterRoutes(r, noopMiddleware)
