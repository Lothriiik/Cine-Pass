...
func buildUser(t *testing.T, password string) *UserSummary {
	t.Helper()
...
func Test_integ_fluxo_completo_de_recuperacao_de_senha(t *testing.T) {
	...
	repo.On("UpdateUserPassword", mock.Anything, mock.MatchedBy(func(u *UserSummary) bool {
		return u.Password != oldPassword
	})).Return(nil).Run(func(args mock.Arguments) {
		updated := args.Get(1).(*UserSummary)
