...
func newTestService() (*SocialService, *MockSocialRepo, *MockUserProvider, *MockSessionProvider) {
	repo := new(MockSocialRepo)
	userProv := new(MockUserProvider)
	sessionProv := new(MockSessionProvider)
	bus := events.NewEventBus()
	svc := NewService(repo, bus, userProv, sessionProv)
...
func Test_deve_criar_post_com_sucesso(t *testing.T) {
	...
	userProv.On("GetUserByID", mock.Anything, userID).Return(&UserSummary{ID: userID, Username: "test_user"}, nil)

	refID := uint(550)
	post, err := svc.CreatePost(context.Background(), userID, CreatePostRequest{
		PostType:    "REVIEW",
	...
	assert.Equal(t, PostTypeReview, post.PostType)
	assert.Equal(t, "Filme incrível!", post.Content)
...
func Test_deve_limitar_feed_a_50_itens(t *testing.T) {
	...
	_, _, err := svc.GetGlobalFeed(context.Background(), 0, 100)
...
func Test_deve_usar_limite_padrao_quando_valor_invalido(t *testing.T) {
	...
	_, _, err := svc.GetGlobalFeed(context.Background(), 0, -5)
...
func Test_deve_enviar_notificacao_ao_curtir_post_de_outro_usuario(t *testing.T) {
	...
	userProv.On("GetUserByID", mock.Anything, likerID).Return(&UserSummary{
		ID:       likerID,
		Username: "liker",
	}, nil)
...
func Test_deve_nao_notificar_ao_curtir_proprio_post(t *testing.T) {
	...
	userProv.On("GetUserByID", mock.Anything, userID).Return(&UserSummary{
		ID:       userID,
		Username: "self",
	}, nil)
...
func Test_deve_enviar_notificacao_ao_seguir_usuario(t *testing.T) {
	...
	userProv.On("GetUserByID", mock.Anything, followerID).Return(&UserSummary{
		ID:       followerID,
		Username: "follower",
	}, nil)

	following, err := svc.ToggleFollow(context.Background(), followerID, followeeID)
