...
func TestLogMovie_WithGamification(t *testing.T) {
	...
	mp.On("GetMovieDetails", mock.Anything, 550).Return(&MovieDetailSummary{Runtime: 120}, nil)
...
func TestGetMovieDetail(t *testing.T) {
	...
	t.Run("Sucesso ao buscar detalhes", func(t *testing.T) {
		mp.On("GetMovieDetails", ctx, tmdbID).Return(&MovieDetailSummary{ID: 1, Title: "Fight Club"}, nil).Once()
